...
import { NarrativesRepository } from '../../repositories/narratives.repository.ts';
...
async function createNewTables(client: Client): Promise<void> {
  const stmts = [
    ...
    `CREATE TABLE IF NOT EXISTS narratives (id TEXT PRIMARY KEY, snapshot_id TEXT NOT NULL REFERENCES snapshots(id), narrative_type TEXT NOT NULL, severity TEXT NOT NULL, fingerprint TEXT NOT NULL, summary TEXT NOT NULL, metadata_json TEXT, created_at TEXT NOT NULL)`,
  ];
...
describe('NarrativesRepository', () => {
  let client: Client; let db: Database; let repo: NarrativesRepository; let snapshotId: string;

  beforeAll(async () => {
    const dbPath = path.join(os.tmpdir(), `prodmind-test-narr-${randomUUID()}.db`);
    ...
    repo = new NarrativesRepository(db);
  });

  afterAll(() => { client.close(); });

  it('inserts and queries narratives by snapshot', async () => {
    const result = await repo.insert([{ snapshotId, narrativeType: 'EXECUTIVE_SUMMARY', severity: 'HIGH', fingerprint: 'fp-n1', summary: 'Test narrative', metadataJson: null }]);
    expect(result.success).toBe(true);
    if (result.success) expect(result.data).toHaveLength(1);
    const narratives = await repo.queryBySnapshot(snapshotId);
    expect(narratives.length).toBeGreaterThan(0);
  });

  it('queries by severity', async () => {
    const narratives = await repo.queryBySeverity('HIGH');
    expect(narratives.length).toBeGreaterThan(0);
  });

  it('queries by fingerprint', async () => {
    const narrative = await repo.queryByFingerprint('fp-n1');
    expect(narrative).toBeDefined();
  });

  it('queries by type', async () => {
    const narratives = await repo.queryByType('EXECUTIVE_SUMMARY');
    expect(narratives.length).toBeGreaterThan(0);
  });
